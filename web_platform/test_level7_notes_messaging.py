...
class TestLevel7CNotesMessaging(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patient_token = self._register_user(f"patient_{prefix}@test.com", "Pass123!", "PATIENT", "Patient Seven C")
        self.doc_token = self._register_doctor(f"doctor_{prefix}@test.com", "Pass123!", "Dr. Seven C", f"REG-7C-{secrets.token_hex(4)}")
        self.admin_token = self._register_user(f"admin_{prefix}@test.com", "Pass123!", "ADMIN", "Admin Seven C")
