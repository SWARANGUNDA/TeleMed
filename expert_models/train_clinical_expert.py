"""
train_clinical_expert.py — Training & Freezing Pipeline for Clinical Expert Model.

Executes:
1. Load Clinical Dataset & Master Patient Split
...
10. Freeze and Save Artifacts to expert_models/saved_models/clinical_v1/
"""
...
logger = logging.getLogger("expert_models.train_clinical")


def run_clinical_expert_pipeline():
    """Run end-to-end training and freezing pipeline for Clinical Expert."""
    logger.info("==================================================================")
    logger.info("        CLINICAL EXPERT MODEL TRAINING & FREEZING PIPELINE        ")
    logger.info("==================================================================")

    # Step 1: Load dataset & master split
    X, y, splits, approved_predictors = data_loader.load_dataset_for_expert(
        dataset_path=config.CLINICAL_DATASET_PATH,
        schema_filename="clinical_features.json"
    )
    ...
    # Step 2: Fit Leakage-Safe Preprocessor (Preserving NaNs for boosting trees)
    ...
    # Generate Validation Probabilities from best estimator
    val_raw_probs = best_estimator.predict_proba(X_val_prep)

    # Step 6: Tune Classification Thresholds on Validation fold ONLY
    tuned_thresholds = threshold_tuner.tune_expert_thresholds(y_val, val_raw_probs)

    # Step 7: Fit Probability Calibrator on Validation fold
    ...
    logger.info("==================================================================")
    logger.info("CLINICAL EXPERT TEST EVALUATION REPORT (UNTOUCHED TEST SET)")
    logger.info("==================================================================")
    ...
    schema_data = data_loader.load_schema("clinical_features.json")
    manager = artifact_manager.ExpertArtifactManager("clinical", version=config.EXPERT_VERSION)
    ...
    # Save report JSON
    report_file = config.REPORTS_DIR / "clinical" / "clinical_expert_report.json"
    with open(report_file, "w", encoding="utf-8") as f:
        json.dump(test_eval_report, f, indent=2)

    logger.info("Clinical Expert pipeline completed. Artifacts frozen at: %s", saved_path)
    return saved_path


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    run_clinical_expert_pipeline()
