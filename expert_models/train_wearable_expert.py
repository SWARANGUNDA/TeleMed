"""
train_wearable_expert.py — Training & Freezing Pipeline for Wearable Expert Model.

Executes:
1. Load Wearable Dataset & Master Patient Split
...
10. Freeze and Save Artifacts to expert_models/saved_models/wearable_v1/
"""
...
logger = logging.getLogger("expert_models.train_wearable")


def run_wearable_expert_pipeline():
    """Run end-to-end training and freezing pipeline for Wearable Expert."""
    logger.info("==================================================================")
    logger.info("        WEARABLE EXPERT MODEL TRAINING & FREEZING PIPELINE        ")
    logger.info("==================================================================")

    # Step 1: Load dataset & master split
    X, y, splits, approved_predictors = data_loader.load_dataset_for_expert(
        dataset_path=config.WEARABLE_DATASET_PATH,
        schema_filename="wearable_features.json"
    )
    ...
    logger.info("==================================================================")
    logger.info("WEARABLE EXPERT TEST EVALUATION REPORT (UNTOUCHED TEST SET)")
    logger.info("==================================================================")
    ...
    schema_data = data_loader.load_schema("wearable_features.json")
    manager = artifact_manager.ExpertArtifactManager("wearable", version=config.EXPERT_VERSION)
    ...
    report_file = config.REPORTS_DIR / "wearable" / "wearable_expert_report.json"
    with open(report_file, "w", encoding="utf-8") as f:
        json.dump(test_eval_report, f, indent=2)

    logger.info("Wearable Expert pipeline completed. Artifacts frozen at: %s", saved_path)
    return saved_path


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    run_wearable_expert_pipeline()
